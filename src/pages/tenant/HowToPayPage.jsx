                      ...
                      <div className="space-y-6">
                        {paymentInfo.map(info => (
                          <div key={info.id} className="text-sm text-slate-300 border-l-2 border-emerald-500/50 pl-3">
                            <p className="font-bold text-slate-100 mb-1">{info.paymentType === 'BANK' ? t('payments.bankTransfer') : t('payments.digitalWallet')}</p>
                            {info.institutionName && <p><span className="text-slate-400">{info.paymentType === 'BANK' ? t('payments.bank') : t('payments.wallet')}:</span> <span className="text-emerald-50">{info.institutionName}</span></p>}
                            {info.accountHolderName && <p><span className="text-slate-400">{t('payments.accountName')}:</span> <span className="text-emerald-50">{info.accountHolderName}</span></p>}
                            {info.accountNumber && <p><span className="text-slate-400">{t('payments.accountNumber')}:</span> <span className="font-mono text-emerald-300 font-semibold">{info.accountNumber}</span></p>}
                            {info.phoneNumber && <p><span className="text-slate-400">{t('payments.phoneNumber')}:</span> <span className="font-mono text-emerald-300 font-semibold">{info.phoneNumber}</span></p>}
                          </div>
                        ))}
                      </div>
                    </div>
